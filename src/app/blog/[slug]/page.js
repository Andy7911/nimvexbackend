import React from "react";
import Editeur from "../../../components/templates/Editeur";
import { BlocksRenderer } from "@strapi/blocks-react-renderer";
BlocksRenderer
export default async function Page({ params }) {


   const { slug } = await params

   const res = await fetch(`${process.env.NEXT_PUBLIC_API_HOST}/api/blogs?filters[slug][$eq]=${slug}`, {
      headers: {
         'Authorization': "Bearer ",
      },

   }
)

   const response = await res.json()

   console.log(response.data[0].article)

   return (<div className="blog-single container">

      <div className="wysiwyg">
         <h1> {response.data[0].title}</h1>

               <Editeur Article={response.data[0].article}/>
      </div>

   </div>)


}